// Affine IFS Point-Set Fitting — interactive lab
    ...
    function w2s(x, y) {
      return [view.width / 2 + (x - world.cx) * world.scale,
              view.height / 2 - (y - world.cy) * world.scale];
    }
    function s2w(sx, sy) {
      return [(sx - view.width / 2) / world.scale + world.cx,
    ...
    function newTheta(K) {
      ...
      for (let k = 0; k < K; k++) {
        theta.push({
          A: [0.5 + 0.05 * randn(), 0.05 * randn(),
              0.05 * randn(), 0.5 + 0.05 * randn()],
    ...
    function nearestIndex(p, set) {
      ...
      for (let i = 0; i < set.length; i++) {
        const dx = p[0] - set[i][0], dy = p[1] - set[i][1];
        const d = dx * dx + dy * dy;
        if (d < bd) { bd = d; best = i; }
      }
    ...
    function computeLossAndGrad(words, theta) {
      ...
      for (let i = 0; i < Np; i++) {
        let gx = gP[i][0], gy = gP[i][1];
        if (gx === 0 && gy === 0) continue;
        const w = words[i];
        const trace = traces[i];
        for (let s = w.length - 1; s >= 0; s--) {
          const k = w[s];
          const xs = trace[s][0], ys = trace[s][1];
          // dA
          grad[k].A[0] += gx * xs;
          grad[k].A[1] += gx * ys;
          grad[k].A[2] += gy * xs;
    ...
    function drawGrid() {
      ...
      ctx.beginPath();
      const [, sy0] = w2s(0, 0); const [sx0] = w2s(0, 0);
      ctx.moveTo(0, sy0); ctx.lineTo(view.width, sy0);
      ctx.moveTo(sx0, 0); ctx.lineTo(sx0, view.height);
      ctx.stroke();
    }
    ...
    function drawFixedPoints() {
      ...
      for (let k = 0; k < state.theta.length; k++) {
        ...
        // I - A = [[1-a, -b], [-c, 1-d]]
        const m11 = 1 - A[0], m12 = -A[1], m21 = -A[2], m22 = 1 - A[3];
    ...
    function presetSierpinski(n = 1500) {
      // classic chaos game
      const verts = [[-0.6, -0.5], [0.6, -0.5], [0, 0.6]];
      const pts = [];
      let x = 0, y = 0;
    ...
    function initSierpinski3() {
      ...
      $("K").value = 3;
      const verts = [[-0.6, -0.5], [0.6, -0.5], [0, 0.6]];
      state.theta = verts.map(v => ({ A: [0.5, 0, 0, 0.5], b: [0.5 * v[0], 0.5 * v[1]] }));
      cachedKey = "";
    }
    function initBarnsley() {
      ...
      state.theta = [
        ...
        { A: [0.20, -0.26, 0.23, 0.22], b: [0, 0.27] },
    ...
    $("K").addEventListener("change", () => { refreshK(); afterParamChange(); });
    $("N").addEventListener("change", () => { refreshN(); afterParamChange(); });
    $("enum").addEventListener("change", () => {
      state.enumeration = $("enum").value; cachedKey = ""; afterParamChange();
    });
    $("initSmall").addEventListener("click", () => { initSmall(); afterParamChange(); });
    $("initRot").addEventListener("click", () => { initRotations(); afterParamChange(); });
    $("initShrink").addEventListener("click", () => { initShrink(); afterParamChange(); });
    $("initSierp").addEventListener("click", () => { initSierpinski3(); afterParamChange(); });
    $("initBarnsley").addEventListener("click", () => { initBarnsley(); afterParamChange(); });
    $("reset").addEventListener("click", () => {
      initSmall();
      state.iter = 0; state.lossHistory = []; state.adamT = 0;
      state.m = []; state.v = [];
      afterParamChange();
    });
    ...
    $("targetClear").addEventListener("click", () => {
      state.target = []; updateHUD(); draw();
    });
    $("loadPreset").addEventListener("click", () => {
      const v = $("targetPreset").value;
      if (!v) return;
      const map = {
        circle: presetCircle, square: presetSquare, heart: presetHeart,
        spiral: presetSpiral, sierpinski: presetSierpinski,
        fern: presetFern, cantor: presetCantor, grid: presetGrid,
      };
      state.target = map[v]();
      updateHUD();
      draw();
    ...
    view.addEventListener("mousedown", e => {
      if (e.button === 2) { panning = true; panLast = [e.clientX, e.clientY]; drawing = false; }
    });
